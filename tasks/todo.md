...
After 2026-05-01: display chain fixed, calibration re-validated (bake-γ + SNR), regression harness shipped, and the integration-debt items (E.2 FITS import, D.1 calibration GUI) closed. The v1 priority order is now narrower:

1. **B.6 Drizzle GUI exposure + AA pre-filter** — engine is splat-complete (CPU+GPU+8 tests, 2× upsample smoke-tested). GUI is the blocker: expose scale + pixfrac in `LuckyStackSection`. Pre-filter is the BiggSky-warned grid-moiré protection — a Gaussian/Sinc lowpass before splat (real defect, not nice-to-have). Auto-engage (FWHM/2.4 trigger) follows.
2. **Visual-diff in F3** — current harness uses `outputBytes` as a behavioural-identity proxy. Add FFT-energy + sharpness metrics + per-pixel diff vs `TESTIMAGES/biggsky/2026-03-05-0055_5-MPO_Jupiter__lapl6_ap126.png` AS!3 reference. Quantifies image quality, not just calibration drift.
3. **LuckyRunner refactor** (unblocks E.1 AVI lucky-stack + FITS lucky-stack) — abstract over byte / RGB / Float32 frame inputs in the runner. Multi-day; defer until a real user demand surfaces.
4. **B.5 PSS proper, C.2 auto-ROI PSF, G.1/G.2 derotation** — genuinely-new-work items; pick when integration debt is fully clear.
5. **Output FITS** — `Exporter.ExportFormat` doesn't list FITS yet; add ~40 lines so stacked outputs save as `.fits` for PixInsight / Siril round-tripping. Low priority unless a user asks.
