...
- [~] **C.1 Blind deconvolution (v0 — limb-LSF auto-PSF + Wiener)** — `Engine/Pipeline/AutoPSF.swift` estimates Gaussian PSF sigma from the planetary limb's line-spread function (second-moment integration on outer side only, 6-px window, baseline subtraction); `--auto-psf` CLI flag runs Wiener.deconvolve with the estimated sigma after the bare stack writes. Empirical 2026-04-28: σ 3.1-3.5 px on three BiggSky Jupiter SERs; visibly improved band detail vs bare `--sharpen` path. **Open work for full C.1**: iterative joint refinement (re-estimate PSF after first-pass deconv), Moffat / anisotropic PSF, per-tile PSF for C.3.
