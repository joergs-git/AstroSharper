...
After 2026-05-01: display chain fixed, calibration re-validated (bake-γ + SNR), regression harness shipped, and three integration-debt items closed (E.2 FITS import, D.1 calibration GUI, B.6 Drizzle GUI). The v1 priority order is now narrower:

1. **B.6 follow-ups** (AA pre-filter + auto-engage trigger + float scales). The picker is live but those are real defects/nice-to-haves. AA is the heaviest — multi-hour Metal kernel work.
