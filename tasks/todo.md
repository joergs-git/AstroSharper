...
After 2026-05-01 (display chain + harness), the foundation is in good shape and the v1 roadmap is the work. Several `[~]` items below have substantial implementations on disk but need an end-to-end functional audit before they can be ticked. Highest-leverage moves for v1:

1. **Audit + tick the `[~]` items** — for each, confirm CLI + GUI wiring, exercise on a relevant TESTIMAGES capture, then either tick or surface the actual gap. Concrete candidates: B.5 multi-level corr, B.6 Drizzle, D.1 pre-stack calibration, E.2 FITS, F4 SourceReader. Cheap, eliminates uncertainty, and may close out 5+ items if implementations are complete.
2. **Visual-diff in the harness (F3 v1)** — current harness uses outputBytes as a behavioural-identity proxy. Add per-pixel diff / FFT-energy / sharpness metrics so calibration changes can be quantified, not just detected. Use the AS!3 reference output `TESTIMAGES/biggsky/2026-03-05-0055_5-MPO_Jupiter__lapl6_ap126.png` as the comparison anchor.
3. **C.2 PSF from auto-ROI** — extends AutoPSF beyond planetary discs (currently bails on lunar / textured / cropped subjects). Real lift for solar surface deconv.
4. **G.1/G.2 derotation** — niche but high-value for long Jupiter/Saturn capture windows. Has a clear use-case from the BiggSky study.

Avoid: starting a new visible feature without first auditing the `[~]` markers. Easy to ship a 3rd version of something we already have.
...
- [~] **F4 SourceReader protocol** — `Engine/IO/SourceReader.swift` exists (~106 lines). Verify SerReader/AviReader conform and that LuckyStack/Stabilizer/QualityProbe consume the protocol uniformly before ticking — implementation present but end-to-end audit deferred.
...
- [~] **B.5 MultiLevelCorrelation** — multi-level phase correlation with coarse 256² fallback when fine/coarse disagree by >5 px landed in `Align.swift` during the session (per memory). Verify the full PSS-style 2× decimated → fine refine cascade is wired and exercised in the lucky-stack alignment loop before ticking.
- [~] **B.6 Drizzle 1.5×/2× with anti-aliasing pre-filter** — `Engine/Pipeline/Drizzle.swift` exists (~159 lines), wired into `cli/Stack.swift` and `LuckyStack.swift`. Verify that the pre-filter, `pixfrac` default (0.7), and auto-engage-when-undersampled gate are all in place before ticking. Implementation looks substantial but end-to-end visual check on an undersampled capture deferred.
...
- [~] **D.1 Pre-stack calibration** — `Engine/Pipeline/Calibration.swift` exists (~137 lines). Wired into `LuckyStack.decodeFrame` (per session memory) with CLI `--master-dark` / `--master-flat` flags. Verify GUI exposes the dark/flat folder pickers and that master frames are auto-derived from a folder of bias/dark/flat shots before ticking — engine-side present, full UX path needs audit.
...
- [~] **E.2 FITS input + output** — `Engine/IO/Fits.swift` exists (~278 lines, both reader + writer in one file). Verify wired into the file-import path (catalog recognition, frame loader) and the export path before ticking — engine-side present, integration audit deferred.
