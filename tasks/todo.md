...
After the 2026-05-01 audit of all `[~]` items, the picture is clearer. The pattern is consistent: **engines are mostly complete, integration / GUI / wiring is the missing layer**. Concrete priorities for v1:

1. **F4 + E.2 together (couples cleanly)** — wrap `FitsReader` as a class conforming to `SourceReader`, introduce a `SourceReader.open(url:)` factory, replace the 3 hard-coded `SerReader(url:)` instantiations (LuckyStack, QualityProbe, OscDefaults), drop the AVI gate at `Stack.swift:523`, then add "fits"/"fit" to `FileCatalog.supportedExtensions` + 4 readDimensions/Exporter wiring lines. Closes both items + unblocks E.1 (AVI lucky-stack) for free. Estimate: ~150 lines, half a session.
2. **D.1 GUI panel** — engine is done + 16-test covered, just need a settings section with two file pickers + optional "build master from folder" helper. ~150 lines, low risk.
3. **B.6 Drizzle GUI exposure + AA pre-filter** — engine is splat-complete, GUI is the blocker. Expose pixfrac + scale in `LuckyStackSection`. Pre-filter is the BiggSky-warned grid-moiré protection — a Gaussian/Sinc lowpass before splat. AA missing is a real defect; auto-engage (FWHM/2.4 trigger) is nice-to-have.
4. **Visual-diff in F3** — current harness uses `outputBytes` as a behavioural-identity proxy, which catches calibration drift but doesn't quantify image quality. Add FFT-energy + sharpness metrics + per-pixel diff vs `TESTIMAGES/biggsky/2026-03-05-0055_5-MPO_Jupiter__lapl6_ap126.png` AS!3 reference.
5. **B.5 PSS proper + C.2 auto-ROI PSF + G.1/G.2 derotation** — these are the genuinely-new-work items, lower priority until the integration debt above is paid down.

The v1 release path is now mostly **integration polish + GUI surfacing**, not new algorithms. That's a healthier place to ship from than mid-algorithm.
...
- [x] **F3 Regression harness** — `astrosharper validate <dir>` shipped 2026-05-01. Walks for `.ser`, runs analyze + stack on each, diffs metrics against committed baselines under `Tests/Regression/baselines/`. ±2 % tolerance on outputBytes; timing + absolute paths stripped before compare. `--regenerate` for after intentional calibration changes; `--filter` for substring narrowing; `--quiet` for CI. 14 baselines for 7 SERs in TESTIMAGES, all 7 green. **Open work for v1+**: visual-diff PNG vs `TESTIMAGES/biggsky/2026-03-05-0055_5-MPO_Jupiter__lapl6_ap126.png` AS!3 reference; richer metrics (sharpness, SNR-flat, FFT-energy, alignment RMS) — current harness uses outputBytes as a proxy for behavioural identity, which catches calibration drift but doesn't quantify image quality directly.
- [~] **F4 SourceReader protocol** — Audited 2026-05-01. Protocol exists with clean shape (~64 lines real). `SerReader` + `AviReader` conform. **Gap**: 3 hard-coded `SerReader(url:)` instantiations remain in `LuckyStack.swift:649`, `QualityProbe.swift:296`, `OscDefaults.swift:1040`; the `Stack.swift:523` AVI gate explicitly says "AVI lucky-stack lands with the SourceReader refactor." `Stabilizer` is already format-agnostic. To tick: (a) introduce `SourceReader.open(url:)` factory, (b) replace those 3 call sites + the gate, (c) `FitsReader` is currently a static-method enum (can't conform) — wrap as a class so FITS plugs in for free.
...
- [~] **B.6 Drizzle 1.5×/2× with anti-aliasing pre-filter** — Audited 2026-05-01. **Splatting is feature-complete**: Fruchter & Hook with pixfrac (default 0.7) on both CPU reference + GPU kernel, 8 unit tests, smoke run produces correctly-upsampled output (450×458 → 986×1002 at scale=2). **Gaps**: (a) NO anti-aliasing pre-filter — the high-freq grid moiré protection is the missing safeguard the BiggSky doc warned about, (b) NO auto-engagement (FWHM/2.4 undersampling trigger), (c) NO GUI exposure (`LuckyStackSection.swift` has zero "drizzle" references — CLI-only feature today), (d) `--drizzle` flag is integer-only, rejects 1.5× per `Stack.swift:117`.
...
- [~] **D.1 Pre-stack calibration** — Audited 2026-05-01. ~95 % complete. Engine math correct ((light − dark) / flatNorm with NaN-safe edges; `buildMasterDark` / `buildMasterFlat` averagers built in). LuckyStack runs calibration in `decodeFrame` BEFORE quality grading (correct ordering); graceful fallback when masters absent or dimension-mismatched. CLI flags `--master-dark <path>` + `--master-flat <path>` accept TIFFs. 16 unit tests (`CalibrationTests` + `CalibrationPolicyTests`). **Gaps**: (a) GUI panel is missing — zero references to master-dark / master-flat / Calibration in `App/Views/`; CLI-only today, (b) folder-scan master-frame builder is missing — current design assumes pre-built master TIFFs (PixInsight / ASTAP workflow). To tick: add a calibration section to a settings panel with two file pickers + an optional "build master from folder" helper (~150 lines).
...
- [~] **E.2 FITS input + output** — Audited 2026-05-01. **Engine is complete + well-tested**: `FitsReader.read()` parses 2D BITPIX=-32 → host-endian Float32 + metadata; `FitsWriter.write()` serialises with 2880-byte block padding; 7+ round-trip / metadata / error-case unit tests all pass. **Integration is zero-wiring**: (a) `FileCatalog.supportedExtensions` doesn't include "fits" / "fit", (b) `readDimensions()` has no FITS case, (c) `Exporter.ExportFormat` enum doesn't list FITS, (d) CLI `analyze` hard-codes `SerReader(url:)` and would fail on a `.fits` file, (e) Stack CLI doesn't accept `.fits` input. To tick: ~40 lines of wiring once `FitsReader` is wrapped as a class conforming to `SourceReader` (couples to F4).
