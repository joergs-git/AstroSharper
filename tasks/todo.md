...
End-of-2026-05-03 (later): **C.2 + C.4 + A.5 (v0) shipped this session** on top of v0.4.0. AutoPSF now cascades to `AutoPSFAutoROI` for lunar / textured / cropped subjects (default OFF, opt-in via `--auto-psf-roi` and GUI sub-toggle). Tile-size auto-calc wired to BiggSky scope formula via `--auto-tile-size --focal-length-mm N --pixel-pitch-um N [--barlow N]` and matching GUI inputs — overrides AutoAP's heuristic when scope params are supplied. Median HFR (A.5 v0) added to the SER quality scan + HUD (XY-shift sparkline deferred — would need a Stabilizer.Result API change to expose per-frame shifts to AppModel). All 301 unit tests + 6/6 F3 regression baselines still green.

**Top of stack for next session:**

1. **C.2 lunar bracket** — empirical validation of `AutoPSFAutoROI` on a real lunar SER. Run `astrosharper stack TESTIMAGES/biggsky/mond-00_06_53_.ser /tmp/lunar_with_roi.tif --keep 5 --smart-auto --auto-psf-roi` and inspect: (a) does the cascade fire (`AutoPSF: auto-ROI σ=…` log line); (b) is the chosen σ in a sane range (typically 1–4 px on lunar terminator); (c) does the deconvolved output look better than the bare stack. ~10–15 min wall-clock on the 13 GB SER. If the σ looks credible across multiple lunar captures, lift the GUI default-off restriction or add a "lunar"-flagged smart-auto path.

2. **A.5 v1 — XY-shift sparkline.** Now that median HFR is in place, the sparkline is a worthwhile follow-up. Needs: (a) Stabilizer.Result extended with `shifts: [(frameIndex, dx, dy)]`, (b) AppModel `@Published var lastStabilizerShifts: [(Int, SIMD2<Float>)]?` populated on completion, (c) HUD renders the shifts as a small SwiftUI `Path` sparkline (12 px tall × 80 px wide). ~3 hr session. Reuses the existing DriftCache validator — the data is already computed inside Stabilizer.run; it's just not currently surfaced.

3. **AVI lucky-stack smoke test** — refactor compiles + SER unchanged, but no real AVI fixture in TESTIMAGES today. First time someone drops a SharpCap AVI into the GUI, validate the path runs through end-to-end. Likely zero-touch but worth eyeballing the result once.
...
5. **F3 v1.4 polish**: drop more reference images into TESTIMAGES so RMSE fires on more baselines (only 4/7 today); add unit tests for the PSS cascade and drizzle AA pre-filter; bracket-script convenience subcommand.

6. **B.6 polish** (auto-engage FWHM/2.4 trigger + float scales), **D.1 polish** (folder-scan master-frame builder), **A.2 two-stage quality**, **A.3 Strehl supplement** — small wins; none individually session-sized.
