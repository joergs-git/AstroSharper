...
- [x] **D.4 Per-channel atmospheric dispersion correction (Path B)** — `Engine/Pipeline/LuckyStackPerChannel.swift`. Each Bayer channel extracted at half-res (true measured pixels, no demosaic interpolation), independently graded + phase-correlated + accumulated, then recombined with bilinear upsample. CLI `--per-channel`. Empirically validated 2026-04-28 against the BiggSky Jupiter SER: with sharpen on top, per-channel produces correct Jupiter color and visible band structure where baseline+sharpen washes out near-monochrome. ~3.2× runtime cost. v0 lightspeed-only — multi-AP / sigma-clip / drizzle / two-stage are NOT yet wired into the per-channel path (they exist on the standard runner; need to be ported as next-block work).
...
- **2026-04-28** — Path B per-channel runner shipped (`LuckyStackPerChannel.swift` + 3 new Metal kernels: `unpack_bayer16_channel_to_rgba`, `unpack_bayer8_channel_to_rgba`, `lucky_combine_channel_planes`). Dispatcher in `LuckyStack.run` engages on Bayer + `--per-channel`; mono SER falls through. v0 is lightspeed-only — multi-AP / sigma-clip / drizzle / two-stage on the per-channel side is the next-block work. New pure-Swift unit suite `BayerChannelSiteTests` (12 tests) validates the pattern × channel × cell math the kernel implements. 246/246 tests green. Empirical A/B on `jupiter_2022-09-10-0649_2-GB-L-Jup_ZWO ASI224MC.ser` (3749 frames, keep 25%): per-channel + `--sharpen` produces correct Jupiter color and visible band structure where baseline + `--sharpen` washes out into near-monochrome. ~3.2× runtime cost (4.81s vs 1.51s on M2). Closes the bulk of D.4 in the v1.0 plan.
